...
inline __m128d swap(__m128d vec)
{
    return (_mm_shuffle_pd(vec,vec,1));
}
inline __m128d neg_0(__m128d i) {
  return _mm_xor_pd(i, __m128d{-0.,0.});
}
inline int intervalClass(__m128d i) {
   return ((_mm_movemask_pd(_mm_cmple_pd(i, __m128d{0., 0.})) << 2) | _mm_movemask_pd(_mm_cmpge_pd(i, __m128d{0., 0.})));
 }
inline int intervalClass(__m128d x, __m128d y) {
  return i_i(intervalClass(neg_0(x)), intervalClass(neg_0(y)));
}

inline __attribute__((always_inline)) __m128d mult(__m128d lhs, __m128d rhs) {
...
inline __attribute__((always_inline)) __m128d div(__m128d lhs, __m128d rhs) {
  switch (intervalClass(lhs, rhs)){
    case E_E:
    case E_M:
    case E_N0:
...
inline __m128d sub(__m128d a, __m128d b){
    return _mm_add_pd(a,swap(b));
}

inline bool equal(__m128d a, __m128d b){
    return _mm_movemask_pd(_mm_cmpeq_pd(a,b))==3;
}

inline double _leftBound(__m128d a)
{
    return -_mm_cvtsd_f64(a);
}
inline double _rightBound(__m128d a){
...
public:
    // --- Konstruktory ---
    inline Interval() : values(_mm_set_pd(0.0, -0.0)) {}
    inline Interval(double a) : values(_mm_set_pd(a, -a)) {}
    inline Interval(double a, double b) : values(_mm_set_pd(b, -a)) {}
    inline Interval(__m128d vec) : values(vec) {}

    inline Interval operator+(const Interval& other) const {
        
        return Interval(add(this->values, other.values));
    }

    inline Interval operator-(const Interval& other) const {
        return Interval(sub(this->values, other.values));
    }

    inline __attribute__((always_inline)) Interval operator*(const Interval& other) const {
        return Interval(mult(this->values, other.values));
    }

    inline Interval operator/(const Interval& other) const {
        return Interval(div(this->values, other.values));
    }
    
    inline Interval& operator+=(const Interval& other) {
        
        this->values = add(this->values, other.values);
        return *this;
    }

    inline Interval& operator-=(const Interval& other) {
        this->values = sub(this->values, other.values);
        return *this;
    }

    inline Interval& operator*=(const Interval& other) {
        this->values = mult(this->values, other.values);
        return *this;
    }

    inline Interval& operator/=(const Interval& other) {
        this->values = div(this->values, other.values);
        return *this;
    }
    ...
    friend inline bool operator==(const Interval& it, const capd::filib::Interval<double>& other) {
        __m128d filib_val = _mm_set_pd(other.rightBound(), -other.leftBound());
        return equal(it.values, filib_val);
    }

    friend inline bool operator==(const capd::filib::Interval<double>& other, const Interval& it) {
        return (it == other);
    }
    ...
    inline void print() const {
        std::cout << "[" << leftBound() << ", " << rightBound() << "]\n";
    }


    friend inline std::ostream& operator<<(std::ostream& os, const Interval& interval) {
